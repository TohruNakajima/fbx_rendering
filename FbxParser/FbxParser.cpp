...
void FbxParser::LoadCacheRecursive(FbxScene * pScene, FbxAnimLayer * pAnimLayer, const char * pFbxFileName, bool pSupportVBO)
{
	...
	{
		FbxTexture * lTexture = pScene->GetTexture(lTextureIndex);	//��ȡ��ͼ��ρE
		FbxFileTexture * lFileTexture = FbxCast<FbxFileTexture>(lTexture);
		if (lFileTexture && !lFileTexture->GetUserDataPtr())
		{
			// Try to load the texture from absolute path
			const FbxString lFileName = lFileTexture->GetFileName();	//��ȡ��ͼ�ļ�ÁE
			...
			const FbxString lAbsFolderName = FbxPathUtils::GetFolderName(lAbsFbxFileName);	//��ȡ����·��
			if (!lStatus)
			{
...
void FbxParser::ProcessJointsAndAnimations(FbxNode *node)
{
	...
	for (unsigned int deformerIndex = 0; deformerIndex != numOfDeformers; ++deformerIndex) {
		...
		for (unsigned int clusterIndex = 0; clusterIndex != numOfClusters; ++clusterIndex) {
			...
			skeleton.joints[currJointIndex].node = currCluster->GetLink();	//��ȡ��ǰ�Ĺؽ�
